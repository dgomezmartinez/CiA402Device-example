...
//CiA402DeviceICanbus::CiA402DeviceICanbus(string canPort)
//{
//    Init(0,canPort);
...
CiA402DeviceICanbus::CiA402DeviceICanbus(long number, string canPort)
{
    ...
    {
        canPorts[i]={i};
        Init(i,canPort+to_string(i));
    }

}

//Función para inicializar los puertos
long CiA402DeviceICanbus::Init(unsigned int canIndex, string canPort)
{
    int val;

    /* Open both CAN nodes for reading and writing */
    if (canIndex <= canPorts.size())
    {
        canPorts[canIndex] = open(canPort.c_str(), O_RDWR);
    }
    else
    {
        canPorts.resize(canIndex);
        canPorts[canIndex] = open(canPort.c_str(), O_RDWR);
    }
